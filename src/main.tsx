...
function Cart({ cart, setCart }: { cart: Item[]; setCart: React.Dispatch<React.SetStateAction<Item[]>> }) {
  ...
  const go = () => { const productLines = cart.map((item, index) => [`${index + 1}. ${item.p.name}`, `   Cantidad: ${item.qty}`, `   Personalización: ${item.customization || 'Sin personalización'}`, `   Importe: ${money(item.p.price * item.qty)}`].join('\n')).join('\n\n'); window.location.href = whatsappLink(`*TAPLESS.EC · PEDIDO NUEVO* 👋\n\nHola equipo Tapless, quiero confirmar mi pedido:\n\n*PRODUCTOS*\n${productLines}\n\n*RESUMEN*\nSubtotal: ${money(subtotal)}\nEnvío: ${shipping ? money(shipping) : 'Gratis'}\nTotal: ${money(subtotal + shipping)}\n\n*DATOS DE ENTREGA*\nNombre: ${name}\nNegocio: ${business}\nUbicación: ${location}\n\nEnviado desde tapless.ec.`); };
