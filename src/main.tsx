...
function WhatsApp() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 3.3a8.7 8.7 0 0 0-7.45 13.2L3.4 20.6l4.45-1.18A8.7 8.7 0 1 0 12 3.3Z" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
      <path d="M8.45 8.25c.3-.4.8-.48 1.18-.2l1.2.88c.4.28.5.8.22 1.2l-.5.7a6.7 6.7 0 0 0 2.62 2.62l.7-.5c.4-.28.92-.18 1.2.22l.88 1.2c.28.38.2.88-.2 1.18l-.72.54c-.48.36-1.1.46-1.67.27a10.9 10.9 0 0 1-4.5-2.92 10.9 10.9 0 0 1-2.92-4.5c-.19-.57-.09-1.19.27-1.67l.54-.72Z" fill="currentColor" />
    </svg>
  );
}
...
function Header({ count, lang, setLang, onSearch, onCartOpen }: { count: number; lang: Lang; setLang: (x: Lang) => void; onSearch: () => void; onCartOpen: () => void }) {
  ...
  return (
    <>
      ...
      <header className={`${isHome ? 'header header-dark' : 'header'} ${scrolled ? 'header-scrolled' : ''}`}>
        <div className="nav wrap">
          ...
          <div className="nav-actions">
            ...
            <a className="nav-talk" href={wa} target="_blank" rel="noreferrer">{t.contact}<ArrowRight size={14} /></a>
  ...
  return (
    <footer>
      <div className="wrap footer-main">
        <div className="footer-intro"><Link to="/" className="brand" aria-label="tapless.ec"><img src={logoSrc} alt="tapless.ec" /></Link><p>Pequeños puntos de contacto.<br />Grandes señales de confianza.</p><a className="footer-cta" href={wa} target="_blank" rel="noreferrer">Hablar con Tapless <ArrowRight size={15} /></a></div>
        <div className="footer-links"><div><strong>Explorar</strong><Link to="/productos">Productos</Link><Link to="/como-funciona">Cómo funciona</Link><Link to="/opiniones">Opiniones</Link><Link to="/faq">Dudas frecuentes</Link></div><div><strong>Ayuda</strong><a href={wa} target="_blank" rel="noreferrer">WhatsApp</a><Link to="/politica-privacidad">Privacidad</Link><Link to="/terminos">Términos</Link></div><div><strong>Social</strong><a href="https://instagram.com/tapless.ec" target="_blank" rel="noreferrer">Instagram</a><a href="https://www.tiktok.com/@tapless.ec" target="_blank" rel="noreferrer">TikTok</a></div></div>
      </div>
...
function Cart({ cart, setCart }: { cart: Item[]; setCart: React.Dispatch<React.SetStateAction<Item[]>> }) {
  ...
  const go = async () => {
    ...
    setPdfNotice('');
    const now = new Date();
    const localDate = [now.getFullYear(), String(now.getMonth() + 1).padStart(2, '0'), String(now.getDate()).padStart(2, '0')].join('').slice(2);
    const orderId = `TP-${localDate}-${Date.now().toString(36).slice(-4).toUpperCase()}`;
    try {
      const generated = await createOrderPdf({ orderId, date: new Intl.DateTimeFormat('es-EC', { dateStyle: 'long' }).format(now), customer: name, business, location, lines: cart.map((item) => ({ name: item.p.name, qty: item.qty, customization: item.customization || '', amount: money(item.p.price * item.qty) })), subtotal: money(subtotal), shipping: shipping ? money(shipping) : 'Gratis', total: money(subtotal + shipping) });
      downloadPdf(generated.blob, generated.filename);
      setPdfNotice(`Proforma ${orderId} descargada. Abriendo WhatsApp...`);
      const message = `*TAPLESS.EC · PROFORMA ${orderId}* 👋\n\nHola equipo Tapless, quiero confirmar mi pedido.\n\nCliente: ${name}\nNegocio: ${business}\nTotal: ${money(subtotal + shipping)}\n\nLa proforma PDF ya fue descargada con todos los detalles. La adjunto en este chat para continuar.`;
      window.setTimeout(() => window.location.assign(whatsappLink(message)), 450);
    } catch {
